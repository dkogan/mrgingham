...
observations will be required for convergence. But a higher-res camera is
required to reliably detect the corners. Wherever the pattern comes from, it is
/strongly/ recommended to leave a margin around the edges. This reduces the
chances of a false corner detection interfering with the grid finder.
