...
The recommended 10x10 pattern can be printed from this file:
[[chessboard.10x10.pdf]]. Wherever the pattern comes from, it is /strongly/
recommended to leave a margin around the edges. This reduces the chances of a
false corner detection interfering with the grid finder.
